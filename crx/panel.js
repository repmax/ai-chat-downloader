...
function createFrontMatter(titleRaw, created_at = '') {
	...
	const slug = `prt-${(created_at ? created_at.replace(/[^0-9]/g, "") : shortDate).slice(2)}-${hostUrl.replace(/[^a-zA-Z0-9]/g, '').substring(0,2)}${fullUrl.replace(/[^a-zA-Z0-9]/g, '').slice(-4).toUpperCase()}-${condensedTitle}`;
	const frontmatter = `---
title: "${rinseTitle}"
tags: []
